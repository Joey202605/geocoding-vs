...
async function callSmartyAPI(address, authId, authToken) {
    try {
        ...
        // 构建查询参数
        let queryParams = `?auth-id=${encodeURIComponent(authId)}&auth-token=${encodeURIComponent(authToken)}`;
        if (street) queryParams += `&street=${encodeURIComponent(street)}`;
        if (city) queryParams += `&city=${encodeURIComponent(city)}`;
        if (state) queryParams += `&state=${encodeURIComponent(state)}`;
        if (zipcode) queryParams += `&zipcode=${encodeURIComponent(zipcode)}`;

        // 通过本地代理转发请求
        const proxyUrl = `${PROXY_URL}${queryParams}`;
        console.log('[Smarty] 通过代理:', proxyUrl);
...
async function parseAddress() {
    ...
    if (!authId || !authToken) {
        alert('请配置 Smarty Auth ID 和 Auth Token');
        return;
    }
